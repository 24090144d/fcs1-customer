...
export async function GET(req: NextRequest) {
  try {
    ...
    const base = `
      WITH source AS (
        SELECT
          hotel_code,
          CASE
            WHEN LOWER(BTRIM(COALESCE(department_name, ''))) = 'unacknowledged orders'
              THEN COALESCE(NULLIF(BTRIM(assigned_to_department), ''), 'Unknown Department')
            ELSE COALESCE(NULLIF(BTRIM(department_name), ''), NULLIF(BTRIM(assigned_to_department), ''), 'Unknown Department')
          END AS department,
          COALESCE(NULLIF(BTRIM(service_item_category), ''), 'Uncategorized') AS category,
    ...
    const message = error instanceof Error ? error.message : 'Unable to load JO table data.';
