...
type ModalLevel = 'cleaning_types' | 'stay_statuses' | 'attendants' | 'details';
...
type TableRow = HotelRow | CleaningTypeRow | StayStatusRow | AttendantRow | DetailRow;
...
export function CorpCoDrilldownTable({ chainCode, hotelFilter, hotelNames, rootLevel = 'hotels', filters, dark }: Props) {
  const { t } = useI18n();
  ...
  const back = () => {
    ...
    else if (modalLevel === 'attendants') setModalLevel('stay_statuses');
    else if (modalLevel === 'stay_statuses') {
  ...
  const breadcrumb = useMemo(() => {
    ...
    const parts = rootLevel === 'hotels'
      ? [hotelLabel(path.hotel), path.cleaningType, path.stayStatus, path.attendant]
      : [path.cleaningType, path.stayStatus, path.attendant];
  ...
  const HotelTable = ({ rows, root = false }: { rows: HotelRow[]; root?: boolean }) => (
    <table className="min-w-[1040px] w-full"><thead style={{ background: tokens.dashboard.tableHeadBg }}><tr>{headerCells([
      t('dashboard_ui.co_table_hotel', 'Hotel'), t('dashboard_ui.co_table_rooms_cleaned', 'Rooms Cleaned'), t('dashboard_ui.co_table_cleaning_types', 'Cleaning Types'), t('dashboard_ui.co_table_attendants', 'Attendants'), t('dashboard_ui.co_table_credits', 'Credits'), t('dashboard_ui.co_table_average_time', 'Average Time'), t('dashboard_ui.co_table_action', 'Action'),
  ...
  const CleaningTypeTable = ({ rows, root = false }: { rows: CleaningTypeRow[]; root?: boolean }) => (
    <table className="min-w-[1080px] w-full"><thead style={{ background: tokens.dashboard.tableHeadBg }}><tr>{headerCells([
      t('dashboard_ui.co_table_cleaning_type', 'Cleaning Type'), t('dashboard_ui.co_table_cleaning_records', 'Cleaning Records'), t('dashboard_ui.co_table_share_pct', 'Share %'), t('dashboard_ui.co_table_attendants', 'Attendants'), t('dashboard_ui.co_table_avg_time', 'Avg. Time'), t('dashboard_ui.co_table_credits', 'Credits'), t('dashboard_ui.co_table_action', 'Action'),
  ...
  const StayStatusTable = ({ rows }: { rows: StayStatusRow[] }) => (
    <table className="min-w-[1180px] w-full"><thead style={{ background: tokens.dashboard.tableHeadBg }}><tr>{headerCells([
      t('dashboard_ui.co_table_stay_status', 'Stay Status'), t('dashboard_ui.co_table_rooms', 'Rooms'), t('dashboard_ui.co_table_share', 'Share'), t('dashboard_ui.co_table_attendants', 'Attendants'), t('dashboard_ui.co_table_avg_time', 'Avg. Time'), t('dashboard_ui.co_table_credits', 'Credits'), t('dashboard_ui.co_table_behind_target', 'Behind Target'), t('dashboard_ui.co_table_action', 'Action'),
  ...
  const DetailTable = ({ rows }: { rows: DetailRow[] }) => (
    <table className="min-w-[1450px] w-full"><thead className="sticky top-0" style={{ background: tokens.dashboard.tableHeadBg }}><tr>{headerCells([
      t('dashboard_ui.co_table_room', 'Room'), t('dashboard_ui.co_table_floor', 'Floor'), t('dashboard_ui.co_table_service_round', 'Service Round'), t('dashboard_ui.co_table_start', 'Start'), t('dashboard_ui.co_table_complete', 'Complete'), t('dashboard_ui.co_table_time_spent', 'Time Spent'), t('dashboard_ui.co_table_standard', 'Standard'), t('dashboard_ui.co_table_ahead_behind', 'Ahead/Behind'), t('dashboard_ui.co_table_credit', 'Credit'), t('dashboard_ui.co_table_flags', 'Flags'),
    ])}</tr></thead><tbody>{rows.map((row, index) => <tr key={`${row.cleaning_order_no}-${index}`}><td className="px-3 py-2 font-mono text-xs font-semibold whitespace-nowrap" style={tdStyle}>{row.room}</td><td className="px-3 py-2 text-xs" style={tdStyle}>{row.floor}</td><td className="px-3 py-2 text-xs" style={tdStyle}>{row.service_round}</td><td className="px-3 py-2 font-mono text-xs whitespace-nowrap" style={tdStyle}>{formatDateTime(row.start_time)}</td><td className="px-3 py-2 font-mono text-xs whitespace-nowrap" style={tdStyle}>{formatDateTime(row.completed_time)}</td><td className="px-3 py-2 font-mono text-xs whitespace-nowrap" style={tdStyle}>{formatMinutes(row.time_spent_minutes)}</td><td className="px-3 py-2 font-mono text-xs whitespace-nowrap" style={tdStyle}>{formatMinutes(row.standard_minutes)}</td><td className="px-3 py-2 font-mono text-xs whitespace-nowrap" style={tdStyle}>{formatVariance(row.variance_minutes)}</td><td className="px-3 py-2 font-mono text-xs" style={tdStyle}>{formatCredit(row.credit, 2)}</td><td className="px-3 py-2" style={tdStyle}><FlagIcon flag={row.flag} /></td></tr>)}</tbody></table>
  );

  const exportRootCsv = () => {
    if (rootLevel === 'hotels') {
      const headers = [t('dashboard_ui.co_table_hotel', 'Hotel'), t('dashboard_ui.co_table_rooms_cleaned', 'Rooms Cleaned'), t('dashboard_ui.co_table_cleaning_types', 'Cleaning Types'), t('dashboard_ui.co_table_attendants', 'Attendants'), t('dashboard_ui.co_table_credits', 'Credits'), t('dashboard_ui.co_table_average_time', 'Average Time')];
      const rows = (rootRows as HotelRow[]).map((row): CsvValue[] => [hotelLabel(row.name), row.rooms_cleaned, row.cleaning_types, row.attendants, row.credits, formatMinutes(row.avg_time_minutes)]);
      downloadCsvFile(`corp-co-hotel-summary-${chainCode.toLowerCase()}.csv`, headers, rows);
    } else {
      const headers = [t('dashboard_ui.co_table_cleaning_type', 'Cleaning Type'), t('dashboard_ui.co_table_cleaning_records', 'Cleaning Records'), t('dashboard_ui.co_table_share_pct', 'Share %'), t('dashboard_ui.co_table_attendants', 'Attendants'), t('dashboard_ui.co_table_avg_time', 'Avg. Time'), t('dashboard_ui.co_table_credits', 'Credits')];
      const rows = (rootRows as CleaningTypeRow[]).map((row): CsvValue[] => [row.name, row.cleaning_records, `${row.share.toFixed(1)}%`, row.attendants, formatMinutes(row.avg_time_minutes), row.credits]);
      downloadCsvFile(`hotel-co-cleaning-type-summary-${hotelFilter.toLowerCase()}.csv`, headers, rows);
    }
  };
  ...
  const exportModalCsv = () => {
    ...
    let rows: CsvValue[][] = [];
    if (modalLevel === 'cleaning_types') {
      headers = [t('dashboard_ui.co_table_cleaning_type', 'Cleaning Type'), t('dashboard_ui.co_table_cleaning_records', 'Cleaning Records'), t('dashboard_ui.co_table_share_pct', 'Share %'), t('dashboard_ui.co_table_attendants', 'Attendants'), t('dashboard_ui.co_table_avg_time', 'Avg. Time'), t('dashboard_ui.co_table_credits', 'Credits')];
      rows = (modalRows as CleaningTypeRow[]).map((row) => [row.name, row.cleaning_records, `${row.share.toFixed(1)}%`, row.attendants, formatMinutes(row.avg_time_minutes), row.credits]);
    } else if (modalLevel === 'stay_statuses') {
      headers = [t('dashboard_ui.co_table_stay_status', 'Stay Status'), t('dashboard_ui.co_table_rooms', 'Rooms'), t('dashboard_ui.co_table_share', 'Share'), t('dashboard_ui.co_table_attendants', 'Attendants'), t('dashboard_ui.co_table_avg_time', 'Avg. Time'), t('dashboard_ui.co_table_credits', 'Credits'), t('dashboard_ui.co_table_behind_target', 'Behind Target')];
      rows = (modalRows as StayStatusRow[]).map((row) => [row.name, row.rooms, `${row.share.toFixed(1)}%`, row.attendants, formatMinutes(row.avg_time_minutes), row.credits, row.behind_target]);
    } else if (modalLevel === 'attendants') {
      headers = [t('dashboard_ui.co_table_attendant', 'Attendant'), t('dashboard_ui.co_table_rooms', 'Rooms'), t('dashboard_ui.co_table_floors', 'Floors'), t('dashboard_ui.co_table_avg_time', 'Avg. Time'), t('dashboard_ui.co_table_cleaning_credits', 'Cleaning Credits'), t('dashboard_ui.co_table_credits_per_room', 'Credits per Room'), t('dashboard_ui.co_table_behind_target', 'Behind Target')];
      rows = (modalRows as AttendantRow[]).map((row) => [row.name, row.rooms, row.floors, formatMinutes(row.avg_time_minutes), row.cleaning_credits, row.credits_per_room, row.behind_target]);
    } else {
      headers = [t('dashboard_ui.co_table_room', 'Room'), t('dashboard_ui.co_table_floor', 'Floor'), t('dashboard_ui.co_table_service_round', 'Service Round'), t('dashboard_ui.co_table_start', 'Start'), t('dashboard_ui.co_table_complete', 'Complete'), t('dashboard_ui.co_table_time_spent', 'Time Spent'), t('dashboard_ui.co_table_standard', 'Standard'), t('dashboard_ui.co_table_ahead_behind', 'Ahead/Behind'), t('dashboard_ui.co_table_credit', 'Credit'), t('dashboard_ui.co_table_flags', 'Flags')];
      rows = (modalRows as DetailRow[]).map((row) => [row.room, row.floor, row.service_round, formatDateTime(row.start_time), formatDateTime(row.completed_time), formatMinutes(row.time_spent_minutes), formatMinutes(row.standard_minutes), formatVariance(row.variance_minutes), row.credit, flagLabel(row.flag)]);
    }
    const scope = [chainCode, path?.hotel, path?.cleaningType, path?.stayStatus, path?.attendant, modalLevel].filter(Boolean).map((part) => csvSlug(String(part))).join('-');
    downloadCsvFile(`co-${scope}.csv`, headers, rows);
  };
    ...
    <>
      <div className="overflow-hidden" style={{ background: tokens.card.bg, border: `1px solid ${tokens.card.border}`, borderLeft: `4px solid ${tokens.accent}`, borderRadius: '12px' }}>
        <div className="flex items-start justify-between gap-3 px-4 py-3" style={{ borderBottom: `1px solid ${tokens.dashboard.tableCellBorder}` }}>
          <div><h4 className="font-serif font-semibold" style={{ color: tokens.text }}>{rootLevel === 'hotels' ? t('dashboard_ui.co_table_hotel_summary', 'Hotel Cleaning Summary') : t('dashboard_ui.co_table_cleaning_type_summary', 'Cleaning Type Summary')}</h4><p className="mt-1 font-mono text-[0.62rem]" style={{ color: tokens.dashboard.tableMuted }}>{rootLevel === 'hotels' ? t('dashboard_ui.co_table_hierarchy', 'Hotel → Cleaning Type → Stay Status → Attendant → Detail') : t('dashboard_ui.co_table_hotel_hierarchy', 'Cleaning Type → Stay Status → Attendant → Detail')}</p></div>
