...
export function CorpMoDrilldownTable({ chainCode, hotelFilter, hotelNames, rootLevel = 'hotels', from = '', to = '', dark }: Props) {
  const { t } = useI18n();
  const { theme } = useTheme();
  const tokens = getAppThemeTokens(theme, dark);
  const tableCode = rootLevel === 'hotels' ? 'cmot-01' : 'mot-01';
  ...
  const queryUrl = useCallback((level: 'hotels' | ModalLevel, drillPath?: DrillPath) => {
    const query = new URLSearchParams({ level, chain: chainCode, type: 'MO' });
    ...
    return `/api/dashboard/mo-table?${query.toString()}`;
  ...
  useEffect(() => {
    ...
    fetch(queryUrl(rootLevel), { signal: controller.signal })
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(payload.error || 'Unable to load MO table data.');
        setRootRows(payload.rows ?? []);
        if (payload.timezone) setTimezone(payload.timezone);
      })
      .catch((cause: unknown) => {
        if (cause instanceof DOMException && cause.name === 'AbortError') return;
        setError(cause instanceof Error ? cause.message : 'Unable to load MO table data.');
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    fetch(queryUrl(modalLevel, path), { signal: controller.signal })
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(payload.error || 'Unable to load MO drilldown data.');
        setModalRows(payload.rows ?? []);
        if (payload.timezone) setTimezone(payload.timezone);
      })
      .catch((cause: unknown) => {
        if (cause instanceof DOMException && cause.name === 'AbortError') return;
        setError(cause instanceof Error ? cause.message : 'Unable to load MO drilldown data.');
        setModalRows([]);
      })
      .finally(() => {
  ...
  const modalTitle = useMemo(() => {
    if (modalLevel === 'departments') return t('dashboard_ui.mo_table_department_summary', 'Department Summary');
    if (modalLevel === 'categories') return t('dashboard_ui.mo_table_category_summary', 'Category Summary');
    if (modalLevel === 'items') return t('dashboard_ui.mo_table_item_summary', 'Defect Summary');
    return t('dashboard_ui.mo_table_details', 'Maintenance Order Details');
  }, [modalLevel, t]);
  ...
  const formatAverageDuration = (minutes: number | null) => minutes === null ? '—' : `${(minutes / 60).toFixed(1)} h`;
  const formatQuantity = (value: number) => Math.round(Number(value) || 0).toLocaleString();
  const actionLabel = t('dashboard_ui.mo_table_drilldown', 'Drill down');
  ...
  const exportRootCsv = () => {
    const firstHeader = rootLevel === 'hotels'
      ? t('dashboard_ui.mo_table_hotel', 'Hotel')
      : t('dashboard_ui.mo_table_department', 'Department');
    const distinctHeader = rootLevel === 'hotels'
      ? t('dashboard_ui.mo_table_departments', 'Departments')
      : t('dashboard_ui.mo_table_defects', 'Defects');
    const headers = [
      firstHeader,
      t('dashboard_ui.mo_table_total_jobs', 'Total Jobs'),
      t('dashboard_ui.mo_table_completed', 'Completed'),
      t('dashboard_ui.mo_table_delayed', 'Delayed'),
      t('dashboard_ui.mo_table_cancelled', 'Cancelled'),
    ...
    const rows: CsvValue[][] = rootRows.map((row) => [
      ...
      `${row.completion_rate.toFixed(1)}%`, row.distinct_count, formatAverageDuration(row.avg_duration_minutes),
    ]);
    const filename = rootLevel === 'hotels'
      ? `corp-mo-hotel-summary-${chainCode.toLowerCase()}.csv`
      : `hotel-mo-department-summary-${hotelFilter.toLowerCase()}.csv`;
  ...
  const exportModalCsv = () => {
    ...
    if (modalLevel === 'departments' || modalLevel === 'categories') {
      headers = [
        modalLevel === 'departments' ? t('dashboard_ui.mo_table_department', 'Department') : t('dashboard_ui.mo_table_category', 'Category'),
        t('dashboard_ui.mo_table_total_jobs', 'Total Jobs'), t('dashboard_ui.mo_table_completed', 'Completed'),
        t('dashboard_ui.mo_table_delayed', 'Delayed'), t('dashboard_ui.mo_table_cancelled', 'Cancelled'),
        t('dashboard_ui.mo_table_completion_rate', 'Completion Rate'), t('dashboard_ui.mo_table_defects', 'Defects'),
        t('dashboard_ui.mo_table_average_duration', 'Average Duration (Hour)'),
      ];
      rows = (modalRows as SummaryRow[]).map((row) => [row.name, row.total_jobs, row.completed, row.delayed, row.cancelled, `${row.completion_rate.toFixed(1)}%`, row.distinct_count, formatAverageDuration(row.avg_duration_minutes)]);
    } else if (modalLevel === 'items') {
      headers = [
        t('dashboard_ui.mo_table_service_item', 'Service Item'), t('dashboard_ui.mo_table_jobs', 'Jobs'),
        t('dashboard_ui.mo_table_quantity', 'Quantity'), t('dashboard_ui.mo_table_completed', 'Completed'),
        t('dashboard_ui.mo_table_delayed', 'Delayed'), t('dashboard_ui.mo_table_completion_rate', 'Completion Rate'),
        t('dashboard_ui.mo_table_average_duration', 'Average Duration (Hour)'),
      ];
      rows = (modalRows as ItemRow[]).map((row) => [row.name, row.total_jobs, Math.round(Number(row.quantity) || 0), row.completed, row.delayed, `${row.completion_rate.toFixed(1)}%`, formatAverageDuration(row.avg_duration_minutes)]);
    } else {
      headers = [
        t('dashboard_ui.mo_table_maintenance_order', 'Maintenance Order'), t('dashboard_ui.mo_table_created_time', 'Created Time'),
        t('dashboard_ui.mo_table_location', 'Location'),
        t('dashboard_ui.mo_table_status', 'Status'), t('dashboard_ui.mo_table_assigned_to', 'Assigned To'),
        t('dashboard_ui.mo_table_duration', 'Duration'), t('dashboard_ui.mo_table_delay', 'Delay'),
        t('dashboard_ui.mo_table_guest_name', 'Guest Name'),
        t('dashboard_ui.mo_table_completed_by', 'Completed By'),
      ];
      rows = (modalRows as DetailRow[]).map((row) => [row.job_order, formatDate(row.created_datetime), row.location, row.status, row.assigned_to, formatDuration(row.duration_minutes), formatDuration(row.delay_minutes), row.guest_name, row.completed_by]);
    }
  ...
  const SummaryTable = ({ rows, firstLabel, distinctLabel, hotelNamesEnabled = false, rootTable = false }: { rows: SummaryRow[]; firstLabel: string; distinctLabel: string; hotelNamesEnabled?: boolean; rootTable?: boolean }) => (
    <table className="min-w-[1120px] w-full">
      <thead style={{ background: tokens.dashboard.tableHeadBg }}><tr>
        {[firstLabel, t('dashboard_ui.mo_table_total_jobs', 'Total Jobs'), t('dashboard_ui.mo_table_completed', 'Completed'), t('dashboard_ui.mo_table_delayed', 'Delayed'), t('dashboard_ui.mo_table_cancelled', 'Cancelled'), t('dashboard_ui.mo_table_completion_rate', 'Completion Rate'), distinctLabel, t('dashboard_ui.mo_table_average_duration', 'Average Duration (Hour)'), t('dashboard_ui.mo_table_action', 'Action')].map((label) => <th key={label} className="px-3 py-2 text-left font-mono uppercase" style={{ ...thStyle, fontSize: '0.62rem', letterSpacing: '0.06em' }}>{label}</th>)}
      </tr></thead>
      <tbody>{rows.map((row) => <tr key={row.name} onMouseEnter={(e) => { e.currentTarget.style.background = tokens.accentTint; }} onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; }}>
        ...
        <td className="px-3 py-2 font-mono text-xs whitespace-nowrap" style={tdStyle}>{formatAverageDuration(row.avg_duration_minutes)}</td>
        <td className="px-3 py-2" style={tdStyle}><DrillButton name={row.name} onClick={() => rootTable ? openRootRow(row.name) : drill(row.name)} /></td>
      </tr>)}</tbody>
    </table>
  );

  const EmptyState = () => <div className="p-8 text-center font-mono text-xs" style={{ color: tokens.dashboard.tableMuted }}>{error || t('dashboard_ui.mo_table_no_data', 'No maintenance order data found for this selection.')}</div>;
    ...
    <>
      <div className="overflow-hidden" style={{ background: tokens.card.bg, border: `1px solid ${tokens.card.border}`, borderLeft: `4px solid ${tokens.accent}`, borderRadius: '12px' }}>
        <div className="flex items-start justify-between gap-3 px-4 py-3" style={{ borderBottom: `1px solid ${tokens.dashboard.tableCellBorder}` }}>
          <div>
            <TableCodeTitle code={tableCode} title={rootLevel === 'hotels' ? t('dashboard_ui.mo_table_hotel_summary', 'Hotel Maintenance Order Summary') : t('dashboard_ui.mo_table_department_summary', 'Department Summary')} titleColor={tokens.text} codeColor={tokens.accent} background={tokens.chart.codeBg} />
            <p className="mt-1 font-mono text-[0.62rem]" style={{ color: tokens.dashboard.tableMuted }}>{rootLevel === 'hotels' ? t('dashboard_ui.mo_table_hierarchy', 'Hotel → Department → Category → Defects → Detail') : t('dashboard_ui.mo_table_hotel_hierarchy', 'Department → Category → Defects → Detail')}</p>
          </div>
          <ExportButton onClick={exportRootCsv} disabled={rootLoading || rootRows.length === 0} />
        </div>
        <div className="overflow-x-auto">
          {rootLoading ? <div className="p-8 flex items-center justify-center gap-2 font-mono text-xs" style={{ color: tokens.dashboard.tableMuted }}><LoaderCircle size={16} className="animate-spin" /> {t('dashboard_ui.mo_table_loading', 'Loading table data…')}</div>
            : rootRows.length > 0 ? <SummaryTable rows={rootRows} firstLabel={rootLevel === 'hotels' ? t('dashboard_ui.mo_table_hotel', 'Hotel') : t('dashboard_ui.mo_table_department', 'Department')} distinctLabel={rootLevel === 'hotels' ? t('dashboard_ui.mo_table_departments', 'Departments') : t('dashboard_ui.mo_table_defects', 'Defects')} hotelNamesEnabled={rootLevel === 'hotels'} rootTable /> : <EmptyState />}
        </div>
      </div>
          ...
          <div role="dialog" aria-modal="true" aria-label={modalTitle} className="w-full max-w-7xl max-h-[88vh] overflow-hidden shadow-2xl" style={{ background: tokens.card.bg, border: `1px solid ${tokens.card.border}`, borderRadius: '12px', ...dialogStyle }}>
            <div {...dragHandleProps} className="flex cursor-move touch-none select-none items-start justify-between gap-4 px-4 py-3" style={{ borderBottom: `1px solid ${tokens.dashboard.tableCellBorder}` }}>
              <div className="min-w-0">
                <div className="flex items-center gap-2"><button type="button" onClick={back} className="inline-flex items-center gap-1 px-2 py-1 font-mono text-[0.65rem] hover:opacity-75" style={{ color: tokens.accent, border: `1px solid ${tokens.accent}55` }}><ArrowLeft size={13} /> {t('dashboard_ui.mo_table_back', 'Back')}</button><TableCodeTitle code={tableCode} title={modalTitle} titleColor={tokens.text} codeColor={tokens.accent} background={tokens.chart.codeBg} /></div>
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                <button type="button" onClick={() => setModalLevel(null)} aria-label={t('dashboard_ui.mo_table_close', 'Close')} className="p-1.5 hover:opacity-70" style={{ color: tokens.textMuted }}><X size={18} /></button>
              </div>
            </div>
            <div className="max-h-[72vh] overflow-auto">
              {modalLoading ? <div className="p-10 flex items-center justify-center gap-2 font-mono text-xs" style={{ color: tokens.dashboard.tableMuted }}><LoaderCircle size={16} className="animate-spin" /> {t('dashboard_ui.mo_table_loading', 'Loading table data…')}</div>
                : modalRows.length === 0 ? <EmptyState />
                : modalLevel === 'departments' || modalLevel === 'categories' ? <SummaryTable rows={modalRows as SummaryRow[]} firstLabel={modalLevel === 'departments' ? t('dashboard_ui.mo_table_department', 'Department') : t('dashboard_ui.mo_table_category', 'Category')} distinctLabel={t('dashboard_ui.mo_table_defects', 'Defects')} />
                : modalLevel === 'items' ? (
                  <table className="min-w-[1040px] w-full">
                    <thead className="sticky top-0" style={{ background: tokens.dashboard.tableHeadBg }}><tr>{[t('dashboard_ui.mo_table_service_item', 'Service Item'), t('dashboard_ui.mo_table_jobs', 'Jobs'), t('dashboard_ui.mo_table_quantity', 'Quantity'), t('dashboard_ui.mo_table_completed', 'Completed'), t('dashboard_ui.mo_table_delayed', 'Delayed'), t('dashboard_ui.mo_table_completion_rate', 'Completion Rate'), t('dashboard_ui.mo_table_average_duration', 'Average Duration (Hour)'), t('dashboard_ui.mo_table_action', 'Action')].map((label) => <th key={label} className="px-3 py-2 text-left font-mono uppercase" style={{ ...thStyle, fontSize: '0.62rem', letterSpacing: '0.06em' }}>{label}</th>)}</tr></thead>
