// Backfill jo_delay_dur_bkt_item_map (duration bucket → service item → count
// for DELAYED jobs) into stored jo_dashboard_json summaries, aggregated from
// jo_records. Powers jo-02's new drilldown: Delayed Duration Distribution →
// Top Service Items.
//
...
// service item defaults to 'Unknown' when blank/null, matching toStr().
//
// Usage: node scripts/backfill_jo_delay_dur_bkt_item_map.mjs [DATABASE_URL]
...
for (const dash of dashRows) {
  const records = (await client.query(
    `SELECT delay_duration, service_item
       FROM jo_records
  ...
  for (const r of records) {
    ...
    const item = (r.service_item ?? '').trim() || 'Unknown';
    if (!map[bkt]) map[bkt] = {};
    map[bkt][item] = (map[bkt][item] ?? 0) + 1;
    total++;
  }
  ...
  await client.query(
    `UPDATE jo_dashboard_json
        SET generated_json = jsonb_set(generated_json, '{summary,jo_delay_dur_bkt_item_map}', $1::jsonb),
            updated_at = NOW()
      WHERE id = $2`,
