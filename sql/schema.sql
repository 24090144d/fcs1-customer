...
--
-- Name: co_ir_records; Type: TABLE; Schema: public; Owner: -
--

-- CO inspection reports are intentionally stored separately from CO-ACSR
-- records so either dataset can be replaced without affecting the other.
CREATE TABLE IF NOT EXISTS public.co_ir_records (
    ...
    chain_code text,
    hotel_code text,
    module_code text DEFAULT 'co'::text NOT NULL,
    country_code text,
    report_variant text DEFAULT 'IR'::text NOT NULL,
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT co_ir_records_duration_source_check CHECK ((duration_source = ANY (ARRAY['TIMESTAMP_DIFF'::text, 'TURN_OVER_TIME'::text]))),
    CONSTRAINT co_ir_records_report_variant_check CHECK ((report_variant = 'IR'::text)),
    CONSTRAINT co_ir_records_upload_job_id_fkey FOREIGN KEY (upload_job_id) REFERENCES public.upload_jobs(id) ON DELETE CASCADE,
    CONSTRAINT co_ir_records_uploaded_file_id_fkey FOREIGN KEY (uploaded_file_id) REFERENCES public.uploaded_files(id)
);

CREATE INDEX IF NOT EXISTS co_ir_records_scope_idx ON public.co_ir_records USING btree (chain_code, hotel_code, inspection_date);
CREATE INDEX IF NOT EXISTS co_ir_records_job_idx ON public.co_ir_records USING btree (upload_job_id);
CREATE INDEX IF NOT EXISTS co_ir_records_inspector_idx ON public.co_ir_records USING btree (inspector);
CREATE INDEX IF NOT EXISTS co_ir_records_room_status_idx ON public.co_ir_records USING btree (room_status);
CREATE INDEX IF NOT EXISTS co_ir_records_location_idx ON public.co_ir_records USING btree (location);
CREATE INDEX IF NOT EXISTS co_ir_records_result_idx ON public.co_ir_records USING btree (inspection_result);
